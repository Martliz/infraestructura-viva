...
como se llamarían contra AWS de verdad. Solo cambia el `endpoint_url`.

No es una simulación con una base de datos genérica: `aws_setup.py` ejecuta
`create_db_instance`, `create_bucket`, `create_table`, `create_topic`,
`put_metric_alarm`, etc. contra la API real de AWS que expone Floci, y Floci
responde levantando un contenedor real de PostgreSQL detrás para RDS.

## 1. Estructura
docker-prototipo/
├── app.py           # API FastAPI — endpoints por servicio AWS
├── aws_setup.py      # Aprovisiona RDS / S3 / DynamoDB / SNS / SQS / CloudWatch vía boto3
...
- **`floci`**: el emulador de AWS, expuesto en `http://localhost:4566`. Necesita
  ...
  detrás de la API.
- **`app`**: la API FastAPI. Al arrancar, ejecuta `aws_setup.provision_all()` en
  ...
  2. Crea los buckets S3, la tabla DynamoDB, el tema SNS y la cola SQS (con la
     suscripción SNS→SQS).
  3. Crea la instancia RDS PostgreSQL (`create_db_instance`) y espera a que
...
| `POST /cloudwatch/metrica-red?bytes_in=...` | Publica un dato de NetworkIn |
...
Todo el código de `aws_setup.py` y `app.py` usa `boto3` sin nada específico de
Floci salvo el `endpoint_url`. Para apuntar a AWS real:
...
#    FLOCI_ENDPOINT vacío (o eliminada) y credenciales IAM reales en vez de
#    AWS_ACCESS_KEY_ID=test.
```

En AWS real, `aws_setup.ensure_rds()` seguiría funcionando igual: crearía (o
...
- Que la app se conecta a RDS únicamente por los datos que devuelve la propia
  API de AWS (`describe_db_instances`), no por una configuración fija — el
